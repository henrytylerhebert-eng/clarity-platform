...
**As of 2026-07-13** (post manual insurance/benefits verification, branch `feat/insurance-benefits`, stacked on `feat/evidence-repository`; remote `github.com/henrytylerhebert-eng/clarity-platform`, protected `main`, open PRs #7/#8/#9 pending the §3a merge queue). A capability appears in exactly one bucket. "Verified" means it ran in this session.
...
~~Case repository~~ ~~case command service~~ ~~document repository~~ ~~foundation hardening~~ ~~evidence repository~~ ~~manual insurance/benefits verification~~ **all done** (ADR-0003…ADR-0009).

**Issue: "Implement authorization readiness (MVP_ROADMAP Phase 2)"** — a per-case authorization-readiness assessment built from what already exists: recorded benefit quotes (`authorizationRequired`/`notificationRequired`), coverage status, and the existing authorization/readiness contracts in `domain-contracts`. Minimal: no payer submission, no integrations — the deliverable is knowing, per case and coverage, whether authorization is required, what's missing, and recording preparation steps through the established command pattern (`Authorization` model, AuthorizationStatus NOT_STARTED → PREPARING → …). Document-repository backlog items remain tracked as GitHub issues #1–#5. In parallel per the roadmap: merge queue (§3a), and line up 2–3 intake-staff conversations before Phase 5 locks the UI.
