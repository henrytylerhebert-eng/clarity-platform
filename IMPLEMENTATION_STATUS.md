...
**As of 2026-07-14** (post authentication, branch `feat/authentication`, stacked on `feat/authorization-readiness` → `feat/insurance-benefits` → `feat/evidence-repository`; remote `github.com/henrytylerhebert-eng/clarity-platform`, protected `main`, open PRs #7/#8/#9/#10/#11 pending the §3a merge queue). A capability appears in exactly one bucket. "Verified" means it ran in this session.
...
**Issue: "Implement the minimal API (MVP_ROADMAP Phase 4)"** — decide ADR-0012 (Proposed: Fastify thin command mapper, routes 1:1 onto existing envelopes, bearer-session middleware via `AuthenticationService.authenticate`, actor derived exclusively from the verified principal — completing the trusted-roles retirement), then implement with contract tests against clarity_dev. Prerequisites: the §3a merge queue (#7 → #8 → #9 → #10 → #11 → this branch's PR, retargeting each stacked PR as its base merges) and the owner's ADR-0012 sign-off. In parallel: the intake-staff conversations (guide at `docs/product/INTAKE_STAFF_DISCOVERY_GUIDE.md`) before Phase 5.
