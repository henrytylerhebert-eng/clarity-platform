...
**As of 2026-07-11** (post evidence repository, branch `feat/evidence-repository`; remote `github.com/henrytylerhebert-eng/clarity-platform` exists with protected `main` and tag `clarity-foundation-v0.1` pushed). A capability appears in exactly one bucket. "Verified" means it ran in this session.
...
~~Case repository~~ ~~case command service~~ ~~document repository~~ ~~foundation hardening~~ ~~evidence repository and human-review workflow~~ **all done** (ADR-0003…ADR-0008).

**Issue: "Implement manual insurance and benefits verification"** — the first business-value vertical slice: stored insurance-card documents + approved INSURANCE evidence → coverage records, subscriber relationships, eligibility verification, benefits quotes, authorization requirements, patient education records, all audited. Uses the existing schema's InsuranceCoverage / EligibilityVerification / BenefitVerification / Authorization / FinancialEducationRecord models through the established command pattern. Document-repository backlog items are tracked as GitHub issues #1–#5. Per the agreed sequence (repository → command service → documents → **evidence** → insurance/benefits → authorization → API → UI → extraction automation): `EvidenceItem` persistence linked to a `SourceDocument`, human-entered (not AI-extracted) evidence records against the existing `EvidenceCategory`/`EvidenceStatus` enums, review/approval transitions (`CANDIDATE → APPROVED/REJECTED/NEEDS_CLARIFICATION/SUPERSEDED`) through the same command-service pattern, case-ownership + document-ownership checks, and audit events for evidence creation and review decisions. Still no OCR or AI extraction — a human enters the evidence text/values; this issue is the persistence + review workflow only.
