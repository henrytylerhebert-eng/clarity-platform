...
- **Synthetic operations backbone domain slice** (`app/src/domain/operationsBackbone.ts`): source-derived read-only fixture and pure projection helpers for `Facility -> Department -> Role -> Owner -> Event -> Review Gate -> Dashboard Exception`. It validates reporting chains, excludes resolved source events from active dashboard exceptions, preserves review gates instead of treating job titles as permissions, and keeps UR/revenue-cycle review separate from emergency clinical action. Verified: focused app test `npm test -- operationsBackbone.test.ts` passed 5/5. No UI, persistence, API, production permission model, compliance engine, or validated operating dashboard.
