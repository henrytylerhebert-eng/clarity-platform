...
- `app/` crisis-path prototype: guided intake, drafts with prohibited-language guards, hash-chained custody ledger, compliance clocks (demo values), packet builder, simulated routing, bedboard, role-adaptive UX — unit tests 47/47, Playwright smoke 16/16, typecheck + production build pass. Frontend demo only (localStorage).
