...
> **CLARITY CONFORMANCE NOTE (added 2026-09-18, Housekeeping Phase 2B).** This skill is
> generic; Clarity's conventions override every example below.
>
...
> - The authoritative index is `docs/architecture/ADR_INDEX.md`, which must be updated in the
>   **same commit** that creates an ADR.
...
> - Before allocating a number, inspect **all refs**, never `main` alone:
>   `git log --all --diff-filter=A --name-only --pretty=format: -- 'docs/architecture/ADR-*' | grep -o 'ADR-[0-9]\{4\}' | sort -u | tail -1`
>   then take that maximum + 1.
> - An ADR records an owner-ratified decision. Do not self-ratify: status stays `Proposed`
