...
function view(overrides: Partial<AssuranceCaseViewDto> = {}): AssuranceCaseViewDto {
  return {
    ...
    version: 1,
    participants: [
      { userId: contributor.userId, role: "EVIDENCE_CONTRIBUTOR", authorityBasis: null, active: true, grantedAt: "2026-09-13T00:00:00Z" },
      { userId: reviewer.userId, role: "QUALIFIED_REVIEWER", authorityBasis: "Synthetic reviewer authority", active: true, grantedAt: "2026-09-13T00:00:00Z" },
    ],
    applicability: [
      { id: "app-1", status: "APPROVED", rationale: "Synthetic applicability approved.", approvedBy: "owner-1", approvedAt: "2026-09-13T00:00:00Z", version: 1, createdAt: "2026-09-13T00:00:00Z" },
    ],
    sources: [
      { id: "source-1", sourceFamilyKey: "SYN-AUTH", versionLabel: "v1", title: "Synthetic authority", authorityClass: "FEDERAL_REGULATION", citation: "SYN-CITATION", sourceUri: null, effectiveAt: null, currentness: "CURRENT", rightsStatus: "PERMITTED" },
    ],
    documentReferences: [
      { id: "policy-1", kind: "POLICY", referenceKey: "POLICY-1", title: "Synthetic policy", versionLabel: "v1", locator: null, createdAt: "2026-09-13T00:00:00Z" },
      { id: "sop-1", kind: "SOP", referenceKey: "SOP-1", title: "Synthetic SOP", versionLabel: "v1", locator: null, createdAt: "2026-09-13T00:00:00Z" },
    ],
    evidenceExpectations: [
      { id: "expectation-1", code: "MONTHLY_ROUND", prompt: "Provide date, owner, and follow-up status.", requiredKeys: ["roundDate", "owner", "followUpStatus"], createdAt: "2026-09-13T00:00:00Z", submissions: [] },
    ],
    sourceConflicts: [],
    evaluations: [],
    ...overrides,
...
function loadMocks(caseView: AssuranceCaseViewDto) {
  ...
  vi.mocked(apiAssuranceGetHistory).mockResolvedValue([
    { kind: "EVIDENCE", id: "history-evidence", occurredAt: "2026-09-13T00:00:00Z", state: "SUBMITTED", version: 1 },
  ]);
}
...
describe("OperatingAssurance workspace", () => {
  ...
  it("renders the governed trust chain and visibly preserves fail-closed states without a compliant shortcut", async () => {
    const stale = view({
      ...
      evaluations: [
        { id: "eval-stale", evidenceSubmissionId: null, result: "STALE_SOURCE", reasonCodes: ["SOURCE_STALE"], requiresHumanReview: true, createdAt: "2026-09-13T01:00:00Z", revision: 2, reviewDecisions: [] },
      ],
    });
  ...
  it("submits only expectationId and evidence payload, then refreshes server state", async () => {
    ...
    expect(apiAssuranceSubmitEvidence).toHaveBeenCalledWith({
      ...
      payload: { roundDate: "2026-09-13", owner: "Synthetic Owner", followUpStatus: "COMPLETE" },
    });
    await waitFor(() => expect(apiAssuranceGetCase).toHaveBeenCalledTimes(2));
  });

  it("does not claim action success when the post-action refresh fails", async () => {
    const caseView = view();
    await loadCase(contributor, caseView);
    vi.mocked(apiAssuranceGetCase).mockRejectedValueOnce(new ApiError(409, "review_requires_current_evidence"));
    fireEvent.change(screen.getByLabelText("Evidence roundDate"), { target: { value: "2026-09-13" } });
    fireEvent.change(screen.getByLabelText("Evidence owner"), { target: { value: "Synthetic Owner" } });
    fireEvent.change(screen.getByLabelText("Evidence followUpStatus"), { target: { value: "COMPLETE" } });
    fireEvent.click(screen.getByRole("button", { name: "Submit evidence" }));

    expect(await screen.findByRole("alert")).toHaveTextContent("evidence state changed");
  ...
  it("revises evidence with only the prior submission id and payload", async () => {
    ...
    const caseView = view({ evidenceExpectations: [{ ...view().evidenceExpectations[0]!, submissions: [existingSubmission] }] });
    await loadCase(contributor, caseView);
    ...
    expect(apiAssuranceReviseEvidence).toHaveBeenCalledWith({
      ...
      payload: { roundDate: "2026-09-12", owner: "Revised Owner", followUpStatus: "OPEN" },
    });
  });
  ...
  it("keeps machine evaluation human-review gated and sends only the approved evaluate envelope", async () => {
    ...
    expect(apiAssuranceEvaluate).toHaveBeenCalledWith({ caseKey: caseView.caseKey, expectationId: "expectation-1" });
  ...
  it("allows the qualified reviewer UI only when both backend-visible grants are present and sends no authority fields", async () => {
    const caseView = view({
      evaluations: [
        { id: "evaluation-1", evidenceSubmissionId: "submission-1", result: "SUPPORTED", reasonCodes: ["EVIDENCE_COMPLETE"], requiresHumanReview: true, createdAt: "2026-09-13T01:00:00Z", revision: 1, reviewDecisions: [] },
      ],
    });
