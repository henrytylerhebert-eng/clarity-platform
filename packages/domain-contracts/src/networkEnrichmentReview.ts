...
/**
 * Network enrichment — human-review and persistence workflow contracts.
 *
 * Reconciliation note (see the reconciliation report for the full conflict
 * map): this module is the human-review / persistence-workflow half of
 * network enrichment, carried over verbatim from `codex/om/sync-main`
 * packets 1-16 (submit/approve/reject/reconcile commands, review-state
 * transitions, role-alias mapping, audit events, compliance export). It is
 * a sibling to `networkEnrichmentResolution.ts` (the entity-resolution /
 * candidate-data-quality half, carried over from the PR #29 stream), not a
 * replacement for it — this file already had real persistence, an outbox,
 * a review UI, and an API surface built against its shape; nothing here
 * was restarted.
 *
 *   resolution (networkEnrichmentResolution.ts) -> candidate enrichment package
 *     -> review and persistence (this file)
 *     -> future controlled canonical promotion (out of scope here)
 *
 * Known gaps preserved as-is, not fixed by this reconciliation (flagged for
 * a separate follow-up, since this task is a contract split, not a bug
 * fix): the `NetworkReviewPolicy.oneReviewSuffices`/`specializedReviewRequired`
 * fields declared below are policy, but the gateway/service layer
 * (packages/network-enrichment-service, outside this file) does not
 * currently read them before promoting a package to HUMAN_CONFIRMED — one
 * approval promotes regardless of the ALL_DISTINCT dual-review intent. That
 * enforcement gap lives in the service/gateway code, not in these type
 * definitions, and is unchanged by this split.
 */
...
/**
 * Proposed source-role vocabulary, mapped onto real `UserRole` values and
 * enforced at runtime below (`NETWORK_SOURCE_ROLE_ALIAS_MAP`,
 * `resolveCanonicalRolesFromSourceRoles`). Overlaps but is not identical to
 * `NetworkReviewerRole` in networkEnrichmentResolution.ts — see that file's
 * header for the specific value differences. This vocabulary being
 * runtime-enforced while the resolution-side one is explicitly not is a
 * real, unresolved governance gap between the two streams (the resolution
 * stream treats the role mapping as an open owner decision; this one
 * already decided it), preserved here rather than reconciled.
 */
