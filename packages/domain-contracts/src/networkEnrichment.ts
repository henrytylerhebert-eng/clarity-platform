...
/**
 * Network enrichment Phase 1 contracts.
 *
 ...
 * (src/ + contracts/). Pure types, Zod schemas, deterministic normalization,
 * entity resolution, source authority, freshness, conflict detection,
 * review-routing policy, and candidate-package validation only — no I/O, no
 * persistence, no scraping, no external APIs, no canonical mutation.
 *
 * Boundary invariants encoded here (ADR-0014):
 ...
 *
 * Reviewer roles here are the package's PROPOSED review vocabulary, not the
 * schema `UserRole` enum. Mapping them onto real Clarity roles is an open
 * owner decision (same class as the prescreen role-mapping packet); until it
 * is resolved these values gate nothing at runtime.
 */
 ...
 * `UserRole` enum; the mapping is an unresolved owner decision and nothing in
 * the runtime authorizes against these values.
 */
...
export const NETWORK_ENRICHMENT_SCHEMA_VERSION =
  "clarity.network-enrichment.v1" as const;
