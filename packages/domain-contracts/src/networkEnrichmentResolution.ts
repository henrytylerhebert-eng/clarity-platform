 ...
 *
 * ADR-0019 note: this module is the entity-resolution /
 * candidate-data-quality half of network enrichment. It is a sibling to
 * `networkEnrichmentReview.ts` (the human-review/persistence workflow
 * half), not a replacement for it. They model different ends of the same
 * candidate pipeline:
 *
 ...
 *
 * Boundary invariants encoded here and adopted by ADR-0019:
 ...
 * unresolved contract gap, not something ADR-0019 resolves.
 */
...
export function stableNetworkJson(value: unknown): string {
  return JSON.stringify(sortNetworkJsonValue(value)) ?? "null";
}

function sortNetworkJsonValue(value: unknown): unknown {
  if (Array.isArray(value)) return value.map(sortNetworkJsonValue);
  ...
  for (const key of Object.keys(obj).sort()) {
    ...
    if (nested !== undefined) {
      sorted[key] = sortNetworkJsonValue(nested);
    }
  }
  return sorted;
}
...
function identifierState(
  ...
  const byType = (
    ...
    for (const identifier of identifiers ?? []) {
      ...
      const values = grouped.get(type) ?? new Set<string>();
      values.add(identifier.value.toLowerCase());
      grouped.set(type, values);
    }
    return grouped;
  };

  const t = byType(target.identifiers);
  const c = byType(candidate.identifiers);
  let comparable = false;
  let match = false;
  for (const [type, targetValues] of t) {
    const candidateValues = c.get(type);
    if (!candidateValues) continue;
    comparable = true;
    const overlap = [...targetValues].some((value) => candidateValues.has(value));
    if (overlap) {
      match = true;
      continue;
    }
...
export function validateNetworkEnrichmentPackage(pkg: NetworkEnrichmentPackage): string[] {
  ...
  for (const c of pkg.candidates) {
    ...
    if (isUnknownString(c.proposedValue) || isUnknownString(c.normalizedValue)) {
...
function isUnknownString(value: NetworkJsonValue | null): boolean {
