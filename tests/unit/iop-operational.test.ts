...
const records = {
  ...
  enrollments: [
    ...
    { enrollmentId: "INP_001", personToken: "PERSON_001", status: "INPATIENT", startsOn: "2028-01-05", endsOn: "2028-02-01", priorEnrollmentId: "ENR_OLD" },
    { enrollmentId: "ENR_001", personToken: "PERSON_001", status: "ENROLLED", startsOn: "2028-02-01", priorEnrollmentId: "INP_001" },
    { enrollmentId: "ENR_003", personToken: "PERSON_003", status: "ENROLLED", startsOn: "2028-02-01" },
  ...
  planVersions: [
    ...
    { planId: "PLAN_B", enrollmentId: "ENR_003", version: "1.0.0", effectiveFrom: "2028-02-01", prescribedDaysPerWeek: 3 },
  ],
  attendance: (Array.from({ length: 6 }, (_, index) => ({ attendanceId: `ATT_${index + 1}`, enrollmentId: "ENR_001", planId: "PLAN_1", serviceDate: "2028-02-07", scheduled: true, outcome: "ATTENDED" as const })) as Array<{ attendanceId: string; enrollmentId: string; planId?: string; serviceDate: string; scheduled: boolean; outcome: "ATTENDED" | "NO_SHOW" | "APPROVED_CANCELED" | "UNSCHEDULED" }>).concat([
    { attendanceId: "ATT_7", enrollmentId: "ENR_003", planId: "PLAN_B", serviceDate: "2028-02-07", scheduled: true, outcome: "ATTENDED" as const },
    { attendanceId: "ATT_8", enrollmentId: "ENR_003", planId: "PLAN_B", serviceDate: "2028-02-07", scheduled: true, outcome: "ATTENDED" as const },
  ...
  services: [
    ...
    { serviceId: "SERVICE_3", sessionId: "SESSION_2", attendanceId: "ATT_7", enrollmentId: "ENR_003", serviceDate: "2028-02-07", category: "GROUP" as const, units: 1 },
    { serviceId: "SERVICE_4", sessionId: "SESSION_2", attendanceId: "ATT_8", enrollmentId: "ENR_003", serviceDate: "2028-02-07", category: "GROUP" as const, units: 1 },
    { serviceId: "SERVICE_5", sessionId: "SESSION_2", attendanceId: "ATT_3", enrollmentId: "ENR_001", serviceDate: "2028-02-07", category: "NON_GROUP" as const, units: 1 },
  ],
...
describe("synthetic IOP P2 operating records", () => {
  it("preserves enrollment history and derives AT13 through AT16 without billing inference", () => {
    ...
    expect(result).toMatchObject({ enrolledCensus: 2, pendingReferralCount: 1, attendedScheduledVisits: 8, eligibleScheduledVisits: 10, noShows: 2, sessions: 2, patientDays: 2, groupUnits: 4, totalUnits: 5, nonGroupUnits: 1, payableMeals: 10 });
  ...
  it("requires enrollment, plan, attendance, and service records to reconcile at their shared grain", () => {
    expect(() => IopOperationalRecordsSchema.parse({
      ...records,
      enrollments: records.enrollments.map((entry) => entry.enrollmentId === "ENR_001" ? { ...entry, personToken: "PERSON_999" } : entry),
    })).toThrow("Prior enrollment must exist for the same person");
    expect(() => IopOperationalRecordsSchema.parse({
      ...records,
      attendance: [{ ...records.attendance[0], planId: "PLAN_2" }],
    })).toThrow("effective on the service date");
    expect(() => IopOperationalRecordsSchema.parse({
      ...records,
      attendance: [{ ...records.attendance[0], outcome: "UNSCHEDULED" }],
    })).toThrow("Scheduled state must agree");
    expect(() => IopOperationalRecordsSchema.parse({
      ...records,
      services: [{ ...records.services[0], attendanceId: "ATT_NO_1" }],
    })).toThrow("only be recorded for attended visits");
  });
});
