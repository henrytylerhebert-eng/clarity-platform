...
describe("network entity resolution", () => {
  ...
  it("matches identifiers when one repeated identifier value overlaps", () => {
    const result = resolveNetworkEntity(
      {
        name: "Acadiana Recovery Center",
        identifiers: [
          { type: "NPI", value: "1111111111" },
          { type: "NPI", value: "2222222222" },
        ],
      },
      [
        {
    ...
    expect(result.selectedCandidateId).toBe("entity-1");
  });
});

describe("network stable JSON", () => {
  it("sorts object keys recursively and omits undefined object fields", () => {
    expect(stableNetworkJson({ b: 2, a: { z: undefined, c: 3 }, list: [{ y: 2, x: 1 }] })).toBe(
      '{"a":{"c":3},"b":2,"list":[{"x":1,"y":2}]}',
    );
  });
});
...
describe("network candidate-package validation", () => {
  ...
  it("rejects the string Unknown in place of null case-insensitively", () => {
