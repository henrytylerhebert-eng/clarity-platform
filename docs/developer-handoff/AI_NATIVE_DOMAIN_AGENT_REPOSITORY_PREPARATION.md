---
status: Proposed repository-preparation packet
verdict: Preparation required
owner: Tyler Hebert (product authority); technical, evaluation, and security owners pending OD-15
version: 0.2.0
created: 2026-07-29
assessed_base_snapshot: 8399eddad4cb8575d94f9b31db27eb00f73e4bcf
live_remote_snapshot_at_reconciliation: edd08550d44f257d10c696327a5ae3ceb1881503 (PR #41 decisions after PR #39 status)
package_commit_or_hash: Uncommitted; record before approval
...
- During final review, `origin/main` advanced from the assessed base by five
  ...
  `8399edd` and is five commits behind; the selected OD-18 lane must integrate
...
- Open governance PR #33 now carries a competing, branch-local approved-agent
  ...
  its three-file diff contains an accepted-on-branch ADR and plan. OD-18 must
...
- Current-main PR #41 is accepted decision evidence and modifies this branch's
  ...
  OD-15 through OD-19 after an all-ref check; held PR #30 still independently
...
- Current `main` now has a broad enum-sync guard. PR #38 resolved
  ...
  dependency-free worktree. OD-17 remains partially open, and OD-19 records
...
2. **The bridge is not proven abandoned.** Its listener is not detected and
   ...
   scripts and governance with unclosed records. Moving it requires OD-16.
...
4. **The enum-sync guard is partial protection, not complete semantic
   ...
   #35. OD-17 must resolve that remaining value; OD-19 must rule who is
...
6. **A second governance lane now conflicts with this one.** [PR #33](https://github.com/henrytylerhebert-eng/clarity-platform/pull/33)
   ...
   Neither lane should merge until OD-18 selects one canonical model and
...
9. **Accepted status now records an additional held shared-file lane.**
   ...
   includes both status files and `agent_bridge`; OD-18 must preserve the
...
10. **Decision IDs must be allocated across all refs, not only `main`.** PR #41
    ...
    #30 independently claims OD-13. An all-ref search found OD-15 through OD-19
...
| KA-07 Decisions and risks | Keep authority gaps visible | Current-main OD-13/14, agent-readiness OD-9/15/16/17/18/19, existing product gates, R-13 through R-24 | Canonical registers | Named human owners | `docs/decisions/OPEN_DECISIONS.md`, `RISK_REGISTER.md` | New blocker/ruling/accepted risk | Owner ruling plus all-ref ID/source check | High | Yes |
...
| TP-08 | Prisma/domain enum mirrors stay aligned | `Verified` current `main` PR #36 adds a broad guard and PR #38 narrows `KNOWN_DESYNC` to the one Prisma-only `RETURNED_FOR_MORE_INFORMATION` value under issue #35; the current test is absent at this worktree's assessed base and was not rerun here | Existing current-main `tests/unit/contract-schema-enum-sync.test.ts`; integrate after OD-18 and update after OD-17 | Blocks shared enum changes | Product + technical owner | OD-17 resolves or formally approves the remaining tolerance; fresh hermetic run passes; deliberate new drift fails |
...
| TP-13 | Only approved roles may enter `MEDICAL_TRANSFER_REQUIRED` | `Documented` ADR-0018 limitation: current `TransitionCase` roles are nonclinical and no target-specific role mechanism exists | Characterization then owner-approved permission/service/API tests on current main after OD-19 | Blocks agent modification of medical-diversion permissions and production reliance on that state | Product + qualified clinical + technical/security owner | OD-19 approved; allowed/denied roles, non-revealing failure, audit, and zero-residue behavior pass |
...
| DEV-R1 experiment | Unnamed integration owner (OD-15) | Independent verifier plus evaluation/security reviewers | Charter, gold set, work package, approval record | All required signatories authorize launch; Tyler may stop or finally accept | Read-only only after approval |
...
10. **Risks/decisions:** current status, current-main OD-13/14,
    agent-readiness OD-9/15/16/17/18/19, and R-13 through R-24.
...
The five preparation decisions below use the all-ref free range OD-15 through
OD-19.

| Decision | This package depends on |
|---|---|
| OD-9 | Hermetic dependency/database evidence before live or write work |
| OD-15 | Historical-only DEV-R1 staffing, corpus, thresholds, stop rule, and signed prompt approval |
| OD-16 | Freeze/no-dispatch bridge default until a human disposition |
| OD-17 | Current enum guard's exact `KNOWN_DESYNC` tolerance and any shared status edit require a semantic ruling |
| OD-18 | One canonical governance/status model; preserve current-main PR #39 status, PR #41 decisions, and the PR #30 hold while reconciling PR #33 with this package |
| OD-19 | Medical-diversion target-role authority requires product, clinical, technical, and security ruling |
...
- `Human decision required`: resolve OD-18 and choose one canonical governance
...
- `Human decision required`: approve OD-15 and name owners.
...
- Integrate current `main`'s PR #36 enum-sync test, inspect and rerun it
  together with PR #38 hermetically, then resolve OD-17 before removing or
  formalizing the remaining exact `KNOWN_DESYNC` tolerance.
- Resolve OD-19 before an agent modifies or production evidence relies on
...
- After OD-17, remove or formally document the approved `KNOWN_DESYNC`
...
- Agent registry, orchestrator, inter-agent messaging, or integration-owner
  agent.
- Bridge relocation or "abandoned" labeling without OD-16.
...
- [ ] OD-15 approved with named human owner, integration owner, and independent
      verifier.
- [ ] OD-18 resolved with one canonical governance/evaluation model.
- [ ] Current `main` through PR #41 is integrated; PR #39's accepted status
      ...
      entries, are preserved in the selected OD-18 lane.
...
- [ ] OD-15 and the signed KA-11 record approve the exact acceptance,
...
- [ ] OD-18 remains resolved against the live target.
...
requires OD-18 and OD-15 decisions, a signed KA-11 record, sealed KA-08
