...
| OD-13 | CMS/Medicare/Medicaid regulatory reference acquisition — the Phase 1 deep research prompt (`docs/legal/GEMINI_DEEP_RESEARCH_PROMPT_CMS_MEDICARE_MEDICAID.md`) **was executed 2026-09-12**; output is at [CMS_MEDICARE_MEDICAID_REGULATORY_REFERENCE_INDEX.md](../legal/CMS_MEDICARE_MEDICAID_REGULATORY_REFERENCE_INDEX.md). Remaining: who reviews the output before any of it informs a rule | Product owner + counsel (OD-2) + clinical (OD-3) | Any regulatory grounding for readiness, authorization, transfer, or consent logic |
...
| OD-28 | Select the authoritative IOP enrollment/plan, note-audit, charge, and EMR-billable sources; approve source ownership, stable IDs, cutoff/correction semantics, program-grant authority, and the provider-backed RLS gate. See the [IOP source-adapter and program-access packet](IOP_REAL_SOURCE_ADAPTER_AND_PROGRAM_ACCESS_PACKET.md). | IOP operations + revenue cycle + privacy/security + technical | Any real IOP import, connector credential, provider RLS rollout, or production enablement |
