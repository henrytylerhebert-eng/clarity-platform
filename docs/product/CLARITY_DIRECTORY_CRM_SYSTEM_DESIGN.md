...
Clarity Directory CRM is the base portal and network registry for behavioral-health access, medical continuity, recovery, post-acute transition, crisis response, and authorized statutory coordination. It lets organizations set up an account, define locations and service lines, invite personnel into scoped roles, and use the right Clarity workflows for prescreen, referral, packet, placement review, consult, intake, admitted operations, utilization review, continuing care, and discharge planning.
...
Parent Organization
  ...
  -> Referral, Consult, Placement Review, Handoff, Admission, UR, Continuing Care, Discharge, Statutory Review
```
...
### Substance-Use Treatment / Recovery Provider

Users:

- Withdrawal-management intake coordinator
- Residential admissions coordinator
- SUD clinician
- MAT or outpatient clinician
...
- Treatment and recovery directory profile
- Detox / withdrawal-management screening
- Residential, IOP, outpatient, and MAT referral preparation
- Requirements review
- Continuing-care and recovery handoff

Allowed actions:

- Maintain distinct service lines for detox, residential, IOP, outpatient, MAT, peer support, and recovery resources.
- Review a prepared treatment referral candidate.
- Coordinate human-reviewed transitions between levels of care.
- Track requirements and source status without implying enrollment or availability.

Blocked actions:

- Automatic level-of-care determination.
- Live availability or admission claims without a verified operational source.
- Treating a recovery-resource listing as treatment acceptance.
...
### Post-Acute / Residential Provider
...
- Skilled nursing, rehabilitation, long-term care, assisted-living, or memory-care profile
- Care-transition candidate
- Requirements and availability notes
- Verification queue

Allowed actions:

- Appear in disposition and care-transition search.
...
### Physician / Ambulatory Specialist
...
- Primary-care or specialty profile
- Follow-up candidate
- Consult and appointment preparation
- Medication and care-continuity requirements

Allowed actions:

- Appear in primary-care and specialist search.
- Review a prepared follow-up or consultation candidate after live workflows are approved.
- Maintain service, population, coverage, and referral-requirement signals through verification.

Blocked actions:

- Being presented as an emergency, inpatient, or psychiatric receiving destination.
- Live appointment availability claims without a verified source.
- Automatic specialist selection.

### Coroner / Statutory Partner

Users:

- Authorized statutory reviewer
- Investigator
- Restricted records staff
- Office admin

Primary modules:

- Restricted partner profile
- Source-scoped review request
- Legal-status or statutory coordination
- Audit preview

Allowed actions:

- Receive an authorized, minimum-necessary review request.
- Return source-scoped facts through a human-reviewed pathway.
- Coordinate only with approved organizations and roles.

Blocked actions:

- Appearing as a care, referral, bed, or discharge destination.
- Unrestricted case or directory access.
- Clinical, placement, or treatment decisions by Clarity.

Elder-protection and similar statutory partners follow the same restricted-partner pattern but retain their own authority, reporting rules, and data boundary.
...
- Primary care clinic
- Specialty provider
- Coroner / statutory coordination office
...
- Acute hospital -> psychiatric hospital or substance-use treatment provider
- Psychiatric or SUD provider -> skilled nursing, assisted living, primary care, or specialist follow-up
- Authorized partner -> coroner or elder-protection office through a restricted statutory pathway
...
### Workflow E: Substance-Use Treatment And Recovery Routing

```text
Case manager selects the needed level-of-care review
  -> searches detox, residential, IOP, outpatient, MAT, and recovery resources
  -> compares requirements, population, coverage signals, and source status
  -> prepares a treatment or recovery handoff candidate
  -> receiving program performs human screening
```

Required UI:

- Substance-use level-of-care filters
- Treatment versus recovery-resource distinction
- Requirements and exclusion review
- Withdrawal-risk and medical-stability caution
- Continuing-care handoff
- No-live-availability warning

### Workflow F: Discharge Planning And Post-Acute Transition

```text
Discharge planner opens case
  -> selects disposition goal
  -> searches skilled nursing, rehab, long-term care, assisted living, memory care, ambulatory, and community resources
...
### Workflow G: Ambulatory Medical Follow-Up

```text
Discharge planner selects a medical follow-up need
  -> searches primary care and relevant specialists
  -> compares service scope, population, coverage signals, and requirements
  -> prepares an appointment or consult candidate
  -> human scheduler and receiving practice confirm next steps
```

Required UI:

- Primary versus specialty care filters
- Consult reason and continuity-record checklist
- Appointment-status field that defaults to unknown
- Medication and discharge-summary preparation
- Clear non-emergency boundary

### Workflow H: Restricted Statutory Coordination

```text
Authorized user selects a statutory workflow
  -> chooses coroner, elder-protection, or other approved partner
  -> records legal basis and minimum necessary context
  -> queues a restricted review request
  -> human reviewer returns a source-scoped response
```

Required UI:

- Statutory partner label
- Role and organization access check
- Legal-basis and source fields
- Minimum-necessary disclosure preview
- Audit and human-review gate
- No-care-destination warning

### Workflow I: Parent Company Admin
...
| SUD intake / recovery navigator | Limited | Yes | Prepare | Review | No | Respond | Transition | No | Yes | Limited |
| Nursing / residential admissions | No | Read | Review | No | No | Respond | Read | No | Yes | Limited |
| Ambulatory referral coordinator | No | Read | Prepare | No | Prepare | No | No | No | Follow-up | Limited |
| Statutory reviewer | No | Restricted | No | No | No | Restricted | No | No | No | Restricted |
...
| `StatutoryReviewRequest` | Restricted, source-scoped request for an approved statutory partner |
