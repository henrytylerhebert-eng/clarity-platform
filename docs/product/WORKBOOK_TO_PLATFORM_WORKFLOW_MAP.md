...
Updated 2026-09-09 · Owner-accepted workbook baseline and authorized parity scope · Synthetic patient/operating data
...
This map covers **30 workbook functions plus four platform workflows**, with a record/grain register, rule crosswalk, code references, acceptance specifications and a dependency-based build sequence. Tyler accepted the workbook on 2026-09-09 and authorized full parity, MVP/interface updates, and real financial-rate implementation; see the [acceptance record](RESTORED_WORKBOOK_ACCEPTANCE.md). Record names below describe responsibilities, not proof that the corresponding database tables or APIs are implemented.
...
Runtime/release status remains governed by [IMPLEMENTATION_STATUS](../../IMPLEMENTATION_STATUS.md) and the [Product Evidence and Decision Protocol](../governance/PRODUCT_EVIDENCE_AND_DECISION_PROTOCOL.md). Implementation of the mapped parity target is authorized. Live source-system adapters, real patient data, deployment, and autonomous clinical/financial decisions remain separate from that development authority.

## Source versions and verification boundary

| Artifact | Identity | What the evidence supports |
|---|---|---|
| Restored workbook used for this map | /Users/tylerhebert/.codex/visualizations/2026/09/08/01a0828f-894b-74b1-adc9-903347636313/outputs/dunder-mifflin-restored-2026/Dunder Mifflin Hospital - Restored Operations 2026.xlsx; frozen SHA-256 `6e81bd61950c244e607ed03f8b0f13e1a4d0bea366ee7ad7cc54c053ef90de26` | Owner accepted 2026-09-09; current hash rechecked and matches. 62 physical sheets: 60 governed, 54 tables, plus empty Sheet1/Sheet2. |
...
|---|---|---|
| IOP enrollment/attendance/note/charge reconciliation described as wholly absent | Synthetic source-link contracts, an authenticated session-aware UI, API/gateway and persistence models now exist (C08/C09). | The vertical slice remains local and bounded; program binding, database RLS/append-only controls, source authority and real adapters are unresolved. |
...
## Acceptance decision and historical audit compatibility

During the September 8 audit, the original checkout gained an acceptance proposal and OD-18 sequencing changes; their inspected hashes remain historical evidence under `concurrent_working_tree_documents` in the evidence manifest. Tyler's explicit September 9 acceptance supersedes the pending product-acceptance state and the prior prohibition on implementation. The current [acceptance record](RESTORED_WORKBOOK_ACCEPTANCE.md) governs. Native revalidation remains unverified and product acceptance is complete; neither is mislabeled as the other.
...
| <a id="x04"></a>**X04 IOP documentation, charge and EMR reconciliation** | Platform/source workflow | [D05](#d05), [D06](#d06), [D18](#d18), [D22](#d22), [D24](#d24), [D20](#d20) | P04 | IOP reviewer matches plan/attendance/note-audit/charge/billable refs and reviews exceptions before close | [C08](#c08), [C09](#c09) — PARTIAL_REUSE | [AT34](../testing/WORKBOOK_PLATFORM_ACCEPTANCE_MATRIX.md#at34), [AT39](../testing/WORKBOOK_PLATFORM_ACCEPTANCE_MATRIX.md#at39) | P2; Authenticated synthetic UI is wired to persisted import/review/close/read; program binding, database controls, source authority and real sources remain unapproved |
...
<a id="c08"></a>
### C08 — Synthetic IOP link validation and authenticated UI
...
This is the **dependency order for the authorized parity scope**. Reuse completed code. P0 owner acceptance is complete; the remaining P0 work pins source contracts and verifies reusable controls for each increment. Use the accepted workbook definitions and record only unresolved behavior-specific choices. Shared functions expand when their owning slice needs them.

| Phase | Bounded deliverable | Depends on | Exit evidence / decision |
|---|---|---|---|
| P0 — Accepted baseline and technical setup | Owner acceptance complete 2026-09-09. Freeze feature/rule/source versions; confirm reusable aggregate close and staffing loop; define source modes, roles and compatible metric definitions. | Accepted workbook and current code baseline | No unassigned F/N/sheet IDs; AT01, AT26–AT29, AT31–AT33 and AT36–AT39 scoped to the selected proof. Remaining technical checks apply to their dependent behavior, not renewed workbook acceptance. |
...
| P3 — Financial semantics and allowance | Implement verified official Medicare/Medicaid source versions, payer/plan/network/contract applicability, deterministic payment calculations, compatible budgets and source-linked UR/assistance views. | Source/rate work begins alongside P1/P2 after P0 definitions. Activity integration needs the relevant P1 or P2 output; F30→F10→F07, with F21 typed budgets. Louisiana hospital provider identifier pending only for facility-specific applicability. | AT07–AT12, AT30, AT38; AT21 financial comparisons follow compatible labor facts in P4. Test official reference calculations and date/applicability boundaries using synthetic activity; unprovided facility factors/private contracts stay missing. |
...
P1 and P2 can be scheduled independently after their technical prerequisites. The IOP handoff remains an initial increment, while P3 source acquisition, rate versions, payer distinctions, and standalone calculation tests can proceed in parallel. Workbook acceptance is complete and all mapped parity implementation is authorized. Full parity remains unfinished until the corresponding behavior and acceptance evidence are delivered; the existing live source-adapter decisions still apply to real connections.
...
| IOP UI-to-API verification | The workspace is wired to persisted server-authenticated import/review/close/read; its focused UI test mocks the API client, while gateway persistence has separate coverage. | Product + technical | Run an authenticated local end-to-end proof across the UI and API; preserve the synthetic/local boundary and receipt distinction. |
...
| Revenue/benefit applicability | Workbook applied rates include fictional inputs; benefit bands and patient share do not supply actual history/eligibility. Official financial-rate implementation is authorized. A Louisiana hospital profile was selected; provider identifier is pending. | Finance + UR/benefits | Verify official method/source versions and independent expected calculations; resolve provider-specific factors and private contract inputs before showing facility-specific amounts. This does not block shared rate-engine or parity implementation. |
...
The September 8 mapping audit changed no application code, contracts, migrations or reference workbooks. Its code/test inventory is historical, not a claim about changes made after that audit. This September 9 update records owner acceptance and implementation authority; current delivery evidence belongs in implementation status. The design retains each function's owner workflow, record grain, rule mapping, code-presence boundary, acceptance requirement and build dependencies.
