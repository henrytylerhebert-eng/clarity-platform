...
**Status:** IMPLEMENTED (backend). Frontend LOCAL_MOCK_ONLY for the IOP reconciliation
surface specifically (`IopReconciliation.tsx`, 145 LOC + a 25-line test — no
`domain/api` import despite `packages/api-service/src/iopReconciliationRoutes.ts`
existing).
...
**Status:** IMPLEMENTED (backend, this session's newest work) / DESIGNED-ONLY at the
product-definition layer / NOT_FOUND as a distinct frontend surface.
