...
| 8 | **`SHELL-MOBILE-01`: the shell area nav is not clickable at 390px** `NEW 2026-09-20` | The fixed-position "Operating Assurance" button overlaps the `ClarityShell` area nav and intercepts pointer events, so **a mobile user cannot tap between applications.** Confirmed on `main` by probe: desktop click passes, mobile click fails. The 20-test smoke suite never caught it because none of its tests use the shell nav, and F5's mobile pass is obtained by keyboard activation rather than a click |
