...
Current phase: prescreen product slice — package onboarded (PRs #17/#20), Phase 1 contracts + hardening (PRs #19/#27), Phase 2 command service (PR #23, ADR-0013), role mapping RESOLVED + same-org HTTP API slice MERGED (ADR-0014, PR #28); provider-backed Cloud SQL/RLS verification remains the separate gate before Phase 3 prescreen persistence. Parallel governance thread: AI-operating-model Stage 0.4 landed (PR #36); Stage 0.1–0.3 HELD behind PR #30 by owner ruling; Stage 1 (R1 trial) must not start until all of Stage 0 is on main
