...
Current phase: prescreen product slice — package onboarded (PRs #17/#20), Phase 1 contracts + hardening (PRs #19/#27), Phase 2 command service (PR #23, ADR-0013), role mapping RESOLVED + same-org HTTP API slice MERGED (ADR-0014, PR #28); provider-backed Cloud SQL/RLS verification remains the separate gate before Phase 3 prescreen persistence. Parallel governance thread: AI-operating-model Stage 0.4 landed (PR #36); PR #33 (plan + ADR-0017) has all seven review findings fixed and awaits merge; Stage 0.1–0.3 HELD behind PR #30 by owner ruling; Stage 1 (R1 trial) requires Stage 0.1–0.3 on main (0.4 is recommended, NOT required — the plan is explicit) and must replay the VULNERABLE parent revisions (ad1b7e9, and 1470e00's parent), never the fix commits. Third thread: CMS regulatory reference — research prompt written but NOT executed (OD-13), corpus acquisition tool merged (PR #42), per-org policy index captured as a packet only (OD-14)
