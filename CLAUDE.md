...
```
PROJECT STATE: Clarity Platform — updated 2026-08-10
Objective: local, tested, tenant-scoped backend foundation for behavioral-health case workflows (synthetic only)
Current phase: prescreen product slice — package onboarded (PRs #17/#20), Phase 1 contracts + hardening (PRs #19/#27), Phase 2 command service (PR #23, ADR-0013), role mapping RESOLVED + same-org HTTP API slice MERGED (ADR-0014, PR #28); Phase 3 LOCAL persistence (ADR-0016, PR #32, OPEN, branch merged current with main this session) — Prisma gateway, RLS'd tables, restart-durable dev server, plus a post-review fix mapping raw Prisma unique-violation contention to domain errors (closes both Gemini Code Assist findings on #32); provider-backed Cloud SQL/RLS verification remains the separate non-waived gate. Parallel governance thread: AI-operating-model Stage 0.4 landed (PR #36); Stage 0.1–0.3 HELD behind PR #30 by owner ruling; Stage 1 (R1 trial) must not start until all of Stage 0 is on main
...
Deliverables: PR #32 branch merged current with main and reverified: root 435/435 (40 files, includes the enum-sync test now covering the 6 prescreen schema enums it didn't know about pre-merge), app 64/64, lint, typecheck, `prisma validate` pass, zero residue delta (28 orgs/15 cases, unchanged across the session); 145/145 prescreen source-package checksums; tag clarity-foundation-v0.1; ADRs accepted on main: 0001–0014 and 0018 (15 files). ADR-0015, 0016, and 0017 are NUMBERS CLAIMED ON UNMERGED BRANCHES — they have not landed and must not be relied on. Allocate a new ADR number only after checking all refs, never main alone: `git log --all --name-only --pretty=format: -- "docs/architecture/ADR-*"` (note that piping to `sort -u` hides duplicate claims, so inspect per-ref before trusting it)
Open items needing owner input: PR #33's four unresolved P2 findings (incl. an ADR-0017 read-only claim its own plan contradicts); whether MEDICAL_TRANSFER_REQUIRED should be clinician-gated (TransitionCase currently permits only INTAKE_COORDINATOR/ORGANIZATION_ADMIN — no per-target-status role mechanism exists); disposition of PR #30; PR #32's required "verify" CI check did not trigger on its Gemini-fix push and needs a look; **`npm audit --audit-level=high` now reports 2 new high-severity findings repo-wide** (brace-expansion follow-up CVE past PR #37's fix, nanoid <3.3.17) disclosed after PR #37 landed — affects every PR's CI, not just #32; Dependabot PR #44 already proposes the brace-expansion half, nanoid still needs a bump
Next action: owner provides GCP access (gcloud auth login + intended project) for the provider-backed Cloud SQL/RLS gate, or decides prescreen UI scope / cross-org packet, or rules on PR #30 and PR #33; merge Dependabot PR #44 and bump nanoid to restore the repo-wide audit gate; confirm PR #32's CI check runs and passes before merge; no Studio mutation/publication/feature-flag/worker/deployment controls before server authorization and audit boundaries exist
```
