...
Open decisions: OD-1 (missing master package), OD-2 (counsel review), OD-3 (clinical licensing), OD-5 (API hosting), OD-6 (DB hosting/RLS — provider is **Supabase Postgres** per ADR-0022, Accepted 2026-09-13, schema already applied; NOT Cloud SQL, which was never provisioned. Still open: provider-backed tenancy tests and independent security review), OD-7 (pnpm/Turborepo), OD-8 (schema graduation), OD-13 (execute CMS regulatory research), OD-14 (per-org AI-native policy index); plus cross-org prescreen submission/receipt model; prescreen UI scope; `codex/om/sync-main` disposition; whether MEDICAL_TRANSFER_REQUIRED should be clinician-gated.
Next action: Housekeeping Phase 2B is landing — ADR-0015 preserved-as-historical and ADR-0020 extracted from PR #73; remaining Phase 2B work is the ADR index, the superseded-PR closures, and the local-branch archive. Then Phase 3 — issue #31 ledger contention, issue #24 residue (fix suite teardown FIRST, it is what regrows it), and the pending `20260917000100_iop_program_binding` migration. `codex/om/sync-main` stays KEEP — DB/MIGRATION DEPENDENCY until Phase 3; never delete `recovery/machine-only/*`. No Access refactor, no new product capability, and no `clarity_dev` mutation before those land. No Studio mutation/publication/feature-flag/worker/deployment controls before server authorization and audit boundaries exist — this explicitly includes scheduling the regulatory corpus tool.
```
