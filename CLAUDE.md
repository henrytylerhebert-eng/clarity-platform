...
```
PROJECT STATE: Clarity Platform — updated 2026-09-18 (Housekeeping Phase 1 truth repair)
Baseline: origin/main = 5c4c0b9 (2026-09-16, merge of PR #99). Verified in-session via `git rev-parse origin/main` and `gh`, not carried forward from memory. Every claim in this block was re-checked against the repository, GitHub, and local `clarity_dev` on 2026-09-18.
...
ADRs on main: 20 files — 0001–0014, 0016–0019, 0021, 0022. GAPS: ADR-0015 exists only on `codex/om/sync-main` (and its recovery ref); ADR-0020 exists only on `claude/tree-structure-buildout-765db6` (PR #73, open, CONFLICTING). Do not allocate ADR-0023 or reuse 0015/0020 until both are dispositioned. Always check all refs by filename, never main alone.
Open PRs (2026-09-18): 12 — #63, #73, #81, #82, #88, #89, #91, #92, #93, #94, #95, #100. Open issues: #1–#5, #24, #31, #35.
...
Open decisions: OD-1 (missing master package), OD-2 (counsel review), OD-3 (clinical licensing), OD-5 (API hosting), OD-6 (DB hosting/RLS — provider-backed evidence still gated), OD-7 (pnpm/Turborepo), OD-8 (schema graduation), OD-13 (execute CMS regulatory research), OD-14 (per-org AI-native policy index); plus cross-org prescreen submission/receipt model; prescreen UI scope; ADR-0015 and ADR-0020 disposition; `codex/om/sync-main` disposition; whether MEDICAL_TRANSFER_REQUIRED should be clinician-gated.
Next action: Housekeeping Phase 2 — disposition ADR-0015, ADR-0020 / PR #73, `codex/om/sync-main`, and the remaining open PRs. Then Phase 3 — issue #31 ledger contention, issue #24 residue, and the pending `20260917000100_iop_program_binding` migration. No Access refactor, no new product capability, and no `clarity_dev` mutation before those land. No Studio mutation/publication/feature-flag/worker/deployment controls before server authorization and audit boundaries exist — this explicitly includes scheduling the regulatory corpus tool.
```
